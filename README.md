...
12- `soledad.DuplicatedDocumentError`. This happens every time now when I start with my `robin2` dev.p-p account. I had previously accidentally specified `dev.p-p.org` as the `--host` to bind to, and this started happening on my next attempt to start with the correct arg: 1--host 0.0.0.0`. Don't know if that is relevant or not.
